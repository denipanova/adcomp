...
genData<-function(N,degrees,start,noise){
  ...
  # start - how far away from the origin
  ...
  n<- as.vector(start+sqrt(runif(N1))*degrees*degrees2rad)
  d1<- t(rbind(-1*n*cos(n)+runif(N1)*noise, sin(n)*n+runif(N1)*noise, rep(0,N1)))
  
  #the second spiral we indicate by 1 in V3
  n<- as.vector(start+sqrt(runif(N1))*degrees*degrees2rad)
  d2<-t(rbind(n*cos(n)+runif(N1)*noise, -1*sin(n)*n+runif(N1)*noise, rep(1,N1))) 
